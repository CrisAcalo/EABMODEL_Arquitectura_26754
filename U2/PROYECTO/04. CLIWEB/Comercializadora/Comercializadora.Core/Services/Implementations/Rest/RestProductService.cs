    ...
    public class RestProductService : IProductService
    {
        ...
        public async Task<IEnumerable<ProductDto>> ObtenerProductosAsync()
        {
            try
            {
                ...
                var result = await GetAsync<IEnumerable<ProductDto>>("productos") ?? Enumerable.Empty<ProductDto>();
        ...
        public async Task<IEnumerable<ProductDto>> ObtenerProductosPorPrecioAsync(decimal precioMin, decimal precioMax)
        {
            try
            {
                ...
                var result = await GetAsync<IEnumerable<ProductDto>>($"productos/precio?precioMin={precioMin}&precioMax={precioMax}") ?? Enumerable.Empty<ProductDto>();
        ...
        public async Task<ServiceResponse<ProductDto>> ActualizarProductoAsync(ProductDto producto)
        {
            try
            {
                ...
                var result = await PatchAsync<UpdateProductDto, ServiceResponse<ProductDto>>(updateProductDto, $"productos/{producto.ProductoId}");
        ...
        private async Task<TResponse?> PatchAsync<TRequest, TResponse>(TRequest data, string endpoint) where TResponse : class
        {
            var httpClient = _httpClientFactory.CreateClient("ComercializadoraClient");
            string fullUrl = string.Empty;
            ...
            try
            {
                ...
                var jsonRequest = JsonConvert.SerializeObject(data);
                _logger?.LogInformation("PATCH Request: {Url} - Data: {Data}", fullUrl, jsonRequest);
                
                var content = new StringContent(jsonRequest, Encoding.UTF8, "application/json");
                // Usando SendAsync para soporte PATCH explícito
                ...
                var jsonResponse = await response.Content.ReadAsStringAsync();
                _logger?.LogInformation("PATCH Response: {StatusCode} - {Content}", response.StatusCode, jsonResponse);
                
                return JsonConvert.DeserializeObject<TResponse>(jsonResponse);
            }
            catch (Exception ex)
            {
                _logger?.LogError(ex, "Error en PATCH: {Url}", fullUrl);
                return null;
            }
        }
