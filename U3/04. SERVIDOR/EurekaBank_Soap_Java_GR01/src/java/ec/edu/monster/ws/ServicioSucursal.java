...
public class ServicioSucursal {
    ...
    public RespuestaDTO listarSucursales() {
        ...
        try {
            ...
            respuesta.setDatos(sucursales);
        } catch (Exception e) {
            LOGGER.log(Level.SEVERE, "Error al listar sucursales", e);
            respuesta.setExitoso(false);
            respuesta.setMensaje("Error al obtener sucursales: " + e.getMessage());
            respuesta.setCodigoError("SUC001");
        }
        return respuesta;
    }
    ...
    public RespuestaDTO registrarSucursal(@WebParam(name = "sucursal") Sucursal sucursal) {
        RespuestaDTO respuesta = new RespuestaDTO();
        try {
            if (sucursal.getCodigo() == null || sucursal.getNombre() == null) {
                respuesta.setExitoso(false);
            ...
            if (existente != null) {
                respuesta.setExitoso(false);
                respuesta.setMensaje("Ya existe una sucursal con ese código");
